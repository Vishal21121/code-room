...
const RoomJoin = () => {
  ...
  const { data: roomsData, isSuccess, isLoading } = useGetRoomsQuery(userName);
  ...
  const createNewRoomRequest = async () => {
    ...
    if (createOrJoin === "Create") {
      ...
      try {
        ...
        const folderData = await createFolder({ roomName }).unwrap();
        console.log("room folder created", folderData.data.value);
        dispatch(setFileData(folderData.data.value));
      } catch (error) {
  ...
  return (
    <div className="bg-[#22272e] h-screen w-screen flex items-center text-center">
      <div className="w-[25%] h-full p-4 bg-gray-900 flex flex-col gap-8">
        ...
        <hr />
        {isLoading ? (
          <BeatLoader color="#888888" className="mx-auto" />
        ) : (
            ...
            {isSuccess && rooms.length !== 0 ? (
              rooms.map(({ _id, name }) => (
                <button
                  key={_id}
                  id={_id}
                  className="p-2 bg-gray-700 text-white text-sm truncate sm:text-md font-semibold rounded-md hover:ring-2 hover:ring-[#FF6C00]"
                  onClick={enterRoom}
                >
                  {name}
                </button>
      ...
      <div className="border w-[50%] sm:w-[30%] h-fit rounded-lg p-8 mx-auto bg-gray-900 flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-4 items-center">
          ...
          />
          <button
            className="px-8 py-2 bg-[#FF6C00] w-fit rounded-lg text-white text-bold text-lg hover:shadow-4xl"
